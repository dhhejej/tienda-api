...
export class SqliteOrderRepository implements OrderRepository {
  public async findAll(): Promise<Order[]> {
    ...
    for (const r of orderRows) {
      ...
      orders.push(new Order(r.id, items, r.total, r.status, new Date(r.created_at)));
    }
  ...
  public async findById(id: string): Promise<Order | null> {
    ...
    return new Order(r.id, items, r.total, r.status, new Date(r.created_at));
  }

  public async save(order: Order): Promise<void> {
    await queryRun(
      'INSERT OR REPLACE INTO orders (id, total, status, created_at) VALUES (?, ?, ?, ?)',
